...
class IMDB(Dataset):
    def __init__(self, data_dir, split, create_data, **kwargs):
        ...
        self.raw_data_path = os.path.join(data_dir, 'imdb.'+split+'.csv')
        self.data_file = 'imdb.'+split+'.json'
        ...
        if create_data:
            print("Creating new %s imdb data."%split.upper())
            self._create_data()
    ...
    def _split_data(self):
        ...
        valid = df.sample(frac=0.1, random_state=200)
        test = df.drop(valid.index)
        train.to_csv(os.path.join(self.data_dir, 'imdb.train.csv'))
        valid.to_csv(os.path.join(self.data_dir, 'imdb.valid.csv'))
