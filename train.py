...
def epoch_iter(model, data_handler, device, optimizer, data_type):
  ...
  for i in range(data_handler.batches_in_epoch):
    batch = data_handler.load_batch(data_type)
    if model.training:
        optimizer.zero_grad()
    elbo = model(batch.to(device))
    if model.training:
        elbo.backward()
        optimizer.step()
    elbo_loss += elbo.item()
...
def main(ARGS, device):
  data_handler = DataHandler('data/movies_genres.csv', (0.8, 0.1, 0.1), ARGS.batch_size, ARGS.max_sequence_length)
